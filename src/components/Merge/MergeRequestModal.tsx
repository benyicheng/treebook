        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-ink-500 dark:text-ink-400 ml-1">合并说明 (可选)</label>
              <Textarea
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                placeholder="向原作者说明为什么该分支值得被合入官方正典..."
                rows={4}
                className="h-32 bg-ink-50 dark:bg-ink-800 border-none focus:ring-2 focus:ring-accent-500"
              />
            </div>
            ...
            <Button
              ...
              fullWidth
              className="py-4 bg-accent-600 hover:bg-accent-700 shadow-lg shadow-accent-500/20"
            >
              {isSubmitting ? '' : '发起合并请求'}
            </Button>
          </form>
