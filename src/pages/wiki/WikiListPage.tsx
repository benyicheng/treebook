...
import { buttonVariants, Input, Badge } from '../../components/ui';
    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-ink-700/80 p-4 rounded-3xl border border-ink-100 dark:border-ink-600 shadow-sm sticky top-20 z-40 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-ink-50 dark:bg-ink-800 border border-ink-100 dark:border-ink-600 rounded-xl">
            ...
            <select
              className="bg-transparent text-sm font-semibold text-ink-600 dark:text-ink-300 outline-none cursor-pointer"
              value={status}
      ...
      {isLoading ? (
        ...
        <div className="py-32 text-center bg-ink-50 dark:bg-ink-800/30 rounded-[3rem] border-2 border-dashed border-ink-200 dark:border-ink-700">
          ...
          <h3 className="text-2xl font-bold text-ink-800 dark:text-white mb-2">百科尚未编撰</h3>
            ...
            {pages.map((page: any) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-ink-50 dark:bg-ink-800 rounded-xl text-xs font-semibold text-ink-500">
                    {contentTypeIcons[page.contentType]}
                    {contentTypeLabels[page.contentType] || page.contentType}
                  </div>
                  <span className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold ${statusLabels[page.status]?.color || ''}`}>
                    {statusLabels[page.status]?.label || page.status}
                  </span>
                </div>
                <h3 className="text-lg font-bold text-ink-800 dark:text-white mb-2 group-hover:text-accent-500 transition-colors">
                  {page.title}
                </h3>
                {page.summary && (
