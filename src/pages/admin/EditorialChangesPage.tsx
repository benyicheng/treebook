  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900 dark:text-white">编辑改稿</h1>
          <p className="text-sm text-gray-400 mt-2">以变更单方式提交/应用改稿，自动留痕并触发退回工单重提与机审</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            <div className="text-sm font-black text-gray-900 dark:text-white">变更单列表</div>
            <div className="text-xs text-gray-400">{loading ? '加载中…' : `${changes.length} 条`}</div>
          </div>
            ...
            {changes.map((c) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="text-[10px] text-gray-400 shrink-0">{new Date(c.updatedAt).toLocaleString()}</div>
                </div>
              </button>
            ))}
        ...
        <div className="rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            <div className="text-sm font-black text-gray-900 dark:text-white">变更单详情</div>
            <div className="text-xs text-gray-400">{loadingDetail ? '加载中…' : selected ? selected.status : ''}</div>
          </div>

          {!selected && (
            <div className="px-5 py-10 text-sm text-gray-400 text-center">选择左侧变更单查看详情</div>
          )}
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="rounded-xl border border-gray-100 dark:border-gray-800 p-4">
                  <div className="text-xs font-black text-gray-500 mb-2">原文</div>
                  <pre className="text-xs whitespace-pre-wrap break-words text-gray-700 dark:text-gray-200">{selected.original || ''}</pre>
                </div>
                <div className="rounded-xl border border-gray-100 dark:border-gray-800 p-4">
                  <div className="text-xs font-black text-gray-500 mb-2">改稿后</div>
                  <pre className="text-xs whitespace-pre-wrap break-words text-gray-700 dark:text-gray-200">{selected.proposed || ''}</pre>
                </div>
              </div>

              <div className="rounded-xl border border-gray-100 dark:border-gray-800 p-4">
                <div className="text-xs font-black text-gray-500 mb-2">动作留痕</div>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {actionPayloads.map((a: any) => (
                    <div key={a.id} className="text-xs">
                      ...
                      {a.payload && (
                        <pre className="mt-1 text-[11px] whitespace-pre-wrap break-words text-gray-500">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() => void apply()}
                  className="px-4 py-2 rounded-xl bg-blue-600 text-white text-sm font-black disabled:opacity-50"
                >
                  应用改稿
                </button>
              </div>
              {selected.status !== 'submitted' && (
                <div className="text-xs text-gray-400">仅 submitted 状态可应用改稿</div>
              )}
            </div>
          )}
        </div>
