      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-8">
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-black text-gray-900 dark:text-white">选择原著世界观</h1>
              <p className="text-gray-500 mt-2 font-medium">所有的番外创作都需要基于一个现有的主线故事。</p>
            </div>
            <button onClick={() => navigate(-1)} className="p-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 text-gray-400 hover:text-gray-900 transition-all">
              <ArrowLeft size={20} />
            </button>
          </div>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-16 pr-6 py-5 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2rem] shadow-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-medium"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredStories.map(story => (
              <div 
                ...
                className="group bg-white dark:bg-gray-900 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 hover:border-indigo-400 transition-all cursor-pointer flex gap-4 items-center"
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-black text-lg text-gray-900 dark:text-white truncate group-hover:text-indigo-600 transition-colors">{story.title}</h3>
                  <p className="text-xs text-gray-400 mt-1 line-clamp-2 leading-relaxed">{story.description}</p>
                </div>
          ...
          {filteredStories.length === 0 && (
            <div className="text-center py-20">
              <Book className="mx-auto text-gray-200 mb-4" size={64} />
              <p className="text-gray-400 font-bold">没有找到匹配的故事</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      ...
      <header className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-6 py-4 flex items-center justify-between sticky top-0 z-30">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            onClick={() => handleSave()}
            disabled={isSaving}
            className="flex items-center gap-2 px-6 py-2 bg-indigo-600 text-white rounded-xl font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 active:scale-95 disabled:opacity-50"
          >
            <Save size={18} />
            {isSaving ? '保存中...' : '保存'}
          </button>
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 border-r border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-y-auto hidden xl:block">
          <div className="p-6 space-y-8">
            <section>
              ...
              <div className="space-y-3">
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                  <h4 className="font-black text-[10px] text-gray-900 dark:text-white mb-1 uppercase tracking-tighter text-indigo-500">主线世界观</h4>
                  <h4 className="font-black text-sm text-gray-900 dark:text-white mb-2">{originalStory?.title}</h4>
                ...
                {originalBranch && (
                  <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-2xl border border-purple-100 dark:border-purple-800 animate-in fade-in slide-in-from-left-4 duration-500">
                    <h4 className="font-black text-[10px] text-purple-600 dark:text-purple-400 mb-1 uppercase tracking-tighter flex items-center gap-1">
                      <GitBranch size={12} />
                      时空分支设定
                    </h4>
                    <h4 className="font-black text-sm text-gray-900 dark:text-white mb-2">{originalBranch.title}</h4>
            ...
            <section className="bg-indigo-600 rounded-3xl p-6 text-white shadow-xl shadow-indigo-500/20">
            ...
            <div className="p-6 h-full">
               <div className="max-w-4xl mx-auto h-full flex flex-col space-y-4">
                  <input 
                    ...
                    className="w-full bg-transparent text-3xl font-black text-gray-900 dark:text-white outline-none placeholder:text-gray-200"
                    placeholder="请输入番外标题..."
                  />
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-gray-500 uppercase tracking-wider">番外类型</label>
                      <div className="grid grid-cols-3 gap-4">
                        {[
                          ...
                          >
                            <t.icon size={20} className={spinoff.type === t.id ? 'text-indigo-600' : 'text-gray-400'} />
                            <div className="font-black text-sm text-gray-900 dark:text-white">{t.label}</div>
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-gray-500 uppercase tracking-wider">短简介 (Summary)</label>
                      <textarea 
                        value={spinoff.summary}
                        onChange={e => setSpinoff(prev => ({ ...prev, summary: e.target.value }))}
                        rows={4}
                        className="w-full px-6 py-4 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl outline-none focus:ring-2 focus:ring-indigo-500 transition-all resize-none"
