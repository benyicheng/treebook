...
/** "chapter-xxx" → { category, contentId } */
...
const ReadingPathCreatePage: React.FC = () => {
  ...
  /* Fetch story map data on mount */
  useEffect(() => {
    if (!storyId) {
      setFetchError('缺少故事 ID');
      setLoading(false);
      return;
    }
    setLoading(true);
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto py-20 px-4 flex flex-col items-center gap-4">
        ...
        <p className="text-sm text-ink-500">加载故事数据…</p>
      </div>
    );
  }
