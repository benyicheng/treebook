...
const ReadingPathEditPage: React.FC = () => {
  ...
  /* Fetch path detail + story map on mount */
  useEffect(() => {
    if (!id) {
    ...
    setPageLoading(true);
    client
      .get(`/reading-paths/${id}`)
      .then((res) => {
        const data: PathDetail = res.data;
        setPathData(data);
        setTitle(data.title);
        setDescription(data.description || '');

        // Pre-populate selected nodes from existing path
        setSelectedNodes(
          data.nodes.map((n) => ({
        ...
        // Fetch map data for the node picker
        return client.get(`/stories/${data.storyId}/map`);
      })
      .then((mapRes) => {
        if (!mapRes) return;
  ...
  /* Verify ownership when both user and path data are loaded */
  useEffect(() => {
  ...
  if (fetchError) {
    return (
      <div className="max-w-4xl mx-auto py-20 px-4 text-center">
        ...
        <button
          onClick={() => navigate(-1)}
          className="px-6 py-2.5 bg-ink-100 dark:bg-ink-700 rounded-xl text-sm font-bold"
        >
          返回
        </button>
      </div>
    ...
    return (
      <div className="max-w-4xl mx-auto py-20 px-4 text-center">
        ...
        <button
          ...
          className="px-6 py-2.5 bg-accent-500 text-white rounded-xl text-sm font-bold hover:bg-accent-600"
        >
          登录
        </button>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-ink-500">修改阅读路径的名称、简介或调整节点顺序</p>
      </div>
            ...
            <div className="flex flex-wrap gap-2">
              {(['chapter', 'branch', 'spinoff'] as const).map((cat) => {
                ...
                return (
                  <span
                    key={cat}
                    className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold border ${nodeColors[cat]}`}
                  >
                    <Icon size={12} />
                    {count} {categoryLabels[cat]}
                  </span>
                );
              })}
            </div>
              ...
              <div className="space-y-1.5 max-h-96 overflow-y-auto pr-1">
                {selectedNodes.map((node, i) => {
                  ...
                  return (
                    ...
                    >
                      {/* Order number */}
                      <span className="w-6 text-center text-xs font-bold text-ink-400 shrink-0">
                        {i + 1}
                      </span>

                      {/* Category icon */}
                      <span
                        className={`inline-flex items-center justify-center w-7 h-7 rounded-lg border ${colorCls} shrink-0`}
                      >
                        <Icon size={14} />
                      </span>
            ...
            <div className="max-h-60 overflow-y-auto p-2 space-y-1">
              {availableByCategory[pickerTab].length === 0 ? (
                <div className="py-8 text-center">
                  <p className="text-xs text-ink-400">
                    暂无
                    {pickerTab === 'chapter' ? '章节' : pickerTab === 'branch' ? '分支' : '番外'}
                  </p>
                </div>
              ) : (
                availableByCategory[pickerTab].map((node) => {
                  const alreadySelected = selectedNodes.some(
                    (n) => n.contentId === parseMapNodeId(node.id)?.contentId,
                  );
                  return (
                    <button
                      key={node.id}
                    ...
                    >
                      <span
                        className={`inline-flex items-center justify-center w-7 h-7 rounded-lg border shrink-0 ${nodeColors[pickerTab]}`}
                      >
                        {React.createElement(nodeIcons[pickerTab], { size: 14 })}
                      </span>
                      <span className="flex-1 text-sm font-medium text-ink-800 dark:text-white truncate">
                        {node.title}
                      </span>
                      {alreadySelected ? (
      ...
      <div className="flex items-center justify-end gap-3 pt-2 pb-8">
        <button
          onClick={() => navigate(-1)}
          className="px-5 py-2.5 rounded-xl text-sm font-bold text-ink-500 hover:bg-ink-100 dark:hover:bg-ink-700 transition-colors"
        >
          取消
        </button>
        <button
          onClick={handleSubmit}
          disabled={submitting || !title.trim() || selectedNodes.length === 0}
